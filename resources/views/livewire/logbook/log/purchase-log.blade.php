<div>
    <x-slot name="header">Purchase Log</x-slot>
    ...
    <x-forms.m-panel>
        ...
        <div class="w-10/12 mx-auto font-lex space-y-5">
            @if($purchase)
                <div class="w-1/3 text-3xl font-merri">{{$purchase->contact->vname}}</div>
                <div class="w-1/2  flex justify-between">
                    <div class=" text-gray-600 flex justify-between space-x-5"><span
                            class="font-merri ">Invoice No:</span><span class="">{{$purchase->purchase_no}}</span></div>
                    <div class=" text-gray-600 flex justify-between space-x-5"><span
                            class="font-merri ">Invoice Date:</span><span class="">{{$purchase->purchase_date}}</span></div>
                </div>
                <div class="w-1/3 text-gray-600 flex justify-between"><span
                        class="font-merri w-1/3">Total Quantity:</span><span class="w-1/3">{{$purchase->total_qty}}</span>
                </div>
                <div class="w-1/3 text-gray-600 flex justify-between"><span
                        class="font-merri w-1/3">Total Taxable:</span><span
                        class="w-1/3">{{$purchase->total_taxable}}</span></div>
                <div class="w-1/3 text-gray-600 flex justify-between"><span
                        class="font-merri w-1/3">Total GST:</span><span class="w-1/3">{{$purchase->total_gst}}</span></div>
                <div class="w-1/3 text-gray-600 flex justify-between"><span class="font-merri w-1/3">Grand Total:</span><span
                        class="w-1/3">{{$purchase->grand_total}}</span></div>
