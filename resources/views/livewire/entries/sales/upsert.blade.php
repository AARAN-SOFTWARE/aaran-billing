<div>
    ...
    <div class="space-y-5">
                    ...
                    <x-slot name="content">
                        ...
                        <x-tabs.content>
                            <div class="w-full flex gap-5 ">
                                <div class="w-full space-y-5 h-48">
                                    <div>
        ...
        <div class="max-w-6xl mx-auto space-y-5 py-5">
            <div
                class="px-4 font-merri tracking-wider text-orange-600 underline underline-offset-4 underline-orange-500">
                Sales Items
            </div>

        </div>
        <div class="max-w-6xl mx-auto flex  gap-x-1 ">
            <div class="">
        ...
        <div class="max-w-6xl mx-auto">
            <div class="w-full border rounded-lg">
                <div
                    class="bg-zinc-50 text-xs py-4 text-gray-400 border-b-2 border-gray-300 flex justify-between text-center">
                    <div class="w-1/12">#</div>
                    @if(\Aaran\Aadmin\Src\SaleEntry::hasPo_no())
                        <div class="w-1/12">PO</div>
                    @endif
                    @if(\Aaran\Aadmin\Src\SaleEntry::hasDc_no())
                        <div class="w-1/12">DC</div>
                    @endif
                    @if(\Aaran\Aadmin\Src\SaleEntry::hasNo_of_roll())
                        <div class="w-1/12">No Of Rolls</div>
                    @endif
                    <div class="w-1/12">Items</div>
                    @if(\Aaran\Aadmin\Src\SaleEntry::hasColour())
                        <div class="w-1/12">Colour</div>
                    @endif
                    @if(\Aaran\Aadmin\Src\SaleEntry::hasSize())
                        div
                    @endif
                    <div class="w-1/12">Quantity</div>
                    <div class="w-1/12">Rate</div>
                    <div class="w-1/12">Taxable</div>
                    <div class="w-1/12">GST Percent</div>
                    <div class="w-1/12">GST</div>
                    <div class="w-1/12">Sub Total</div>
                    <div class="w-1/12">Action</div>
                </div>

                @if ($itemList)
                    @foreach($itemList as $index => $row)
                        <div class="flex justify-between text-center text-xs border-b py-4">
                            <div class="w-1/12" wire:click.prevent="changeItems({{$index}})">{{$index+1}}</div>
                            @if(\Aaran\Aadmin\Src\SaleEntry::hasPo_no())
                                <div class="w-1/12" wire:click.prevent="changeItems({{$index}})">{{$row['po_no']}}</div>
                            @endif
                            @if(\Aaran\Aadmin\Src\SaleEntry::hasDc_no())
                                <div class="w-1/12" wire:click.prevent="changeItems({{$index}})">{{$row['dc_no']}}</div>
                            @endif
                            @if(\Aaran\Aadmin\Src\SaleEntry::hasNo_of_roll())
                                <div class="w-1/12"
                                     wire:click.prevent="changeItems({{$index}})">{{$row['no_of_roll']}}</div>
                            @endif
                            <div class="w-2/12" wire:click.prevent="changeItems({{$index}})">
                            ...
                            @if(\Aaran\Aadmin\Src\SaleEntry::hasSize())
                                <div class="w-1/12"
                                     wire:click.prevent="changeItems({{$index}})">{{$row['size_name']}}</div>
                            @endif
                            <div class="w-1/12" wire:click.prevent="changeItems({{$index}})">{{$row['qty']}}</div>
                            <div class="w-1/12" wire:click.prevent="changeItems({{$index}})">{{$row['price']}}</div>
                            <div class="w-1/12" wire:click.prevent="changeItems({{$index}})">{{$row['taxable']}}</div>
                            <div class="w-1/12"
                                 wire:click.prevent="changeItems({{$index}})">{{$row['gst_percent']}}</div>
                            <div class="w-1/12"
                                 wire:click.prevent="changeItems({{$index}})">{{$row['gst_amount']}}</div>
                            <div class="w-1/12" wire:click.prevent="changeItems({{$index}})">{{$row['subtotal']}}</div>
                            <div class="w-1/12">
                    ...
                    @endforeach
                @else
                    <div class="w-full text-xs border-b-2 py-4 text-center font-merri">No Data</div>
                @endif
                <div class="flex justify-between text-xs text-gray-400 py-4">
                    @if(\Aaran\Aadmin\Src\SaleEntry::hasSize() or \Aaran\Aadmin\Src\SaleEntry::hasColour())
                        <td colspan="" class="">&nbsp;TOTALS&nbsp;&nbsp;&nbsp;</td>
                    @else
                        <td colspan="" class="">&nbsp;TOTALS&nbsp;&nbsp;&nbsp;</td>
                    @endif

                    <div class="">{{$total_qty}}</div>
                    <div class="">&nbsp;</div>
                    <div class="">{{$total_taxable}}</div>
                    <div class="">&nbsp;</div>
                    <div class="">{{$total_gst}}</div>
                    <div class="">{{$grandtotalBeforeRound}}</div>
                    <div class="">&nbsp;</div>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-6xl mx-auto">
            <x-tabs.tab-panel>
                ...
                <x-slot name="content">
                    ...
                    <x-tabs.content>
                        <div class="flex justify-between gap-5 h-48">
                            <div class="w-full space-y-5 ">
                    ...
                    <x-tabs.content>
                        <div class="space-y-5 gap-5 h-48">
        ...
        <div class="max-w-6xl mx-auto flex justify-between items-center gap-5">
            <div class="w-full">
            ...
            <div class="w-full flex text-xs text-400">
