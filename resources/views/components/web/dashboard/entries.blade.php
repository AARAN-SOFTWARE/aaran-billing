...
<div class="sm:w-4/12 w-auto bg-white  rounded-lg flex-col flex h-[28rem] hover:shadow-md">
    ...
    <div class="w-full h-[4rem] py-3 border-b border-gray-200 inline-flex items-center justify-between px-8">
        <span class="inline-flex items-center gap-2">
            ...
            <span class="font-semibold text-lg font-lex">Entries</span>
        </span>
    </div>
    ...
    <div class="flex-col flex h-[24rem] px-2 overflow-y-auto">
        ...
        <a href="{{route('sales')}}">
            <x-web.dashboard.entries_list entry="Sales" date="{{$entries['sales_date']}}" invoice="INV - {{$entries['sales_no']}}" amount="{{$entries['sales']}}" color="text-[#23B7E5]">
                <x-icons.sales />
            </x-web.dashboard.entries_list>
        </a>

        <a href="{{route('purchase')}}">
            <x-web.dashboard.entries_list entry="Purchase" date="{{$entries['purchase_date']}}" invoice="INV - {{$entries['purchase_no']}}" amount="{{$entries['purchase']}}" color="text-[#845ADF]">
                <x-icons.purchase />
            </x-web.dashboard.entries_list>
        </a>

        <a href="{{route('transactions',[2])}}">
            <x-web.dashboard.entries_list entry="Payment" date="{{$entries['payment_date']}}" invoice="" amount="{{$entries['payment']}}" color="text-[#E6533C]">
                <x-icons.payment />
            </x-web.dashboard.entries_list>
        </a>

        <a href="{{route('transactions',[1])}}">
            <x-web.dashboard.entries_list entry="Receipt" date="{{$entries['receipt_date']}}" invoice="" amount="{{$entries['receipt']}}" color="text-[#F5B849]">
                <x-icons.receipt />
            </x-web.dashboard.entries_list>
        </a>
    </div>
