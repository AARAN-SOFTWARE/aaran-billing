<div class="mt-12">
    <div class="w-8/12 h-screen mx-auto flex-col flex gap-6 justify-evenly items-center">
        <div class="font-roboto text-4xl font-bold flex justify-center py-2 animate__animated wow bounceInDown"
             data-wow-duration="3s">A Software packed with great features
        </div>

        <div style="background-image: url('/../../../images/dashboard.png')"
             class="relative mt-8 w-full mx-auto bg-no-repeat bg-cover h-[30rem] rounded-xl shadow-lg shadow-gray-300 animate__animated wow bounceInDown"
             data-wow-duration="3s">

            <div
                class="absolute -top-12 -left-6 flex-col flex justify-center items-center gap-4 animate__animated wow animate__backInLeft"
                data-wow-duration="3s">
                <div
                    class="w-60 p-3 text-white text-xs font-roboto tracking-wider bg-[#2846f0]/80 shadow shadow-gray-200 rounded-md">
                    Multi user access from any PC or tablet
                </div>
                <div class="relative">
                    <div
                        class="w-9 h-9 rounded-full bg-[#2846f0]/40 animate-ping transition  duration-[5000] delay-1000 ease-in  ">

                    </div>
                    <div
                        class="absolute top-1 left-1 w-7 h-7 rounded-full bg-[#2846f0] text-white font-bold font-roboto flex items-center justify-center text-xs">
                        1
                    </div>
                </div>
            </div>
            <div
                class="absolute bottom-16 left-48 flex-col flex justify-center items-center gap-4 animate__animated wow animate__backInLeft"
                data-wow-duration="3s">
                <div
                    class="w-60 p-3 text-white text-xs font-roboto tracking-wider bg-[#2846f0]/80 shadow shadow-gray-200 rounded-md">
                    Personalized and optimized templates
                </div>
                <div class="relative">
                    <div
                        class="w-9 h-9 rounded-full bg-[#2846f0]/40 animate-ping transition  duration-[5000] delay-1000 ease-in  ">

                    </div>
                    <div
                        class="absolute top-1 left-1 w-7 h-7 rounded-full bg-[#2846f0] text-white font-bold font-roboto flex items-center justify-center text-xs">
                        2
                    </div>
                </div>
            </div>
            <div
                class="absolute bottom-64 right-56 flex-col flex justify-center items-center gap-4 animate__animated wow animate__backInRight"
                data-wow-duration="3s">
                <div
                    class="w-60 px-3 py-4 text-white text-xs font-roboto tracking-wider bg-[#2846f0]/80 shadow shadow-gray-200 rounded-md">
                    Keep records of stock in real time
                </div>
                <div class="relative">
                    <div
                        class="w-9 h-9 rounded-full bg-[#2846f0]/40 animate-ping transition  duration-[5000] delay-1000 ease-in  ">

                    </div>
                    <div
                        class="absolute top-1 left-1 w-7 h-7 rounded-full bg-[#2846f0] text-white font-bold font-roboto flex items-center justify-center text-xs">
                        3
                    </div>
                </div>
            </div>
            <div
                class="absolute bottom-28 right-80 flex-col flex justify-center items-center gap-4 animate__animated wow animate__backInRight"
                data-wow-duration="3s">
                <div
                    class="w-60 p-3 text-white text-xs font-roboto tracking-wider bg-[#2846f0]/80 shadow shadow-gray-200 rounded-md">
                    Amazing GST updated billing software
                </div>
                <div class="relative">
                    <div
                        class="w-9 h-9 rounded-full bg-[#2846f0]/40 animate-ping transition  duration-[5000] delay-1000 ease-in  ">

                    </div>
                    <div
                        class="absolute top-1 left-1 w-7 h-7 rounded-full bg-[#2846f0] text-white font-bold font-roboto flex items-center justify-center text-xs">
                        4
                    </div>
                </div>
            </div>

        </div>
        <div class="grid grid-cols-2 gap-12 text-sm text-gray-500 font-roboto tracking-wider py-4">
            <div class="animate__animated wow animate__backInLeft" data-wow-duration="3s">
                Codexsun is the best rated top seller for a reason. It comes with a lot of great features
                and available resources, but at the same time we always try to keep things as simple and intuitive as possible, so
                that users with little experience have no problem using it as well.
            </div>
            <div class="animate__animated wow animate__backInRight" data-wow-duration="3s">
                With a cloud based GST billing software, you can access all your documents and create invoices on the
                go. It works on any operating system, computer or tablet, and it's completely secure!, we care
                that your site runs as smooth as possible. You got a question? Don’t hesitate to get in touch.
            </div>
        </div>
    </div>
</div>
